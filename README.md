...
| **Local backup & restore** *(on by default)* | Adds a button to the Backup screen that opens Nutrilio's built-in (normally hidden) local file backup/restore. Exports everything — all entries, images and settings — to a portable `.nutrilio` file (a ZIP) via the share sheet, and restores one from a file you pick. No Google account or premium needed. | ✅ Applies cleanly; pending on-device confirmation |
...
- Or download the bundle directly:
  [`patches-1.12.0.mpp`](https://github.com/durgesh0505/chiggi_morphe_patches/releases/latest)
...
```bash
java -jar morphe-cli.jar list-patches --patches=patches/build/libs/patches-1.12.0.mpp -v
java -jar morphe-cli.jar patch -p patches/build/libs/patches-1.12.0.mpp -o out.apk base.apk
```
